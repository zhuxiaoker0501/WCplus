...
class DB:
    ...
    def insert(self, key, data, l111111ll1_wcplus_=True):
        ...
        if l111111ll1_wcplus_:
            if type(data) == dict:
                res = self._replace_into_one(key, data)
            else:
                if type(data) == list:
                    res = self._replace_into_many(key, data)
                else:
                    if type(data) == dict:
    ...
    def _replace_into_one(self, key, data):
        """
        :param key: 判存字段
        :param data: {}
    ...
    def _replace_into_many(self, key, data):
        """
        :param key: 判存字段
        :param data: []
        ...
        res = 'INSERT'
        for d in data:
            if self._replace_into_one(key, d) == 'UPDATE':
                res = 'UPDATE'
